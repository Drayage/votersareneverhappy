...
const effectSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("discardForBudget"), max: z.number().min(1), tag: tagSchema.optional(), per: z.number().optional() }),
  z.object({ kind: z.literal("trashForScore") }),
  z.object({ kind: z.literal("trashForDraw") }),
  z.object({ kind: z.literal("trashForBudget") }),
  z.object({ kind: z.literal("discardForCostScore"), mult: z.number().min(1) }),
  z.object({ kind: z.literal("digTagForBudget"), tag: tagSchema, max: z.number().min(1) }),
  z.object({ kind: z.literal("topDeckGamble"), tag: tagSchema, bonus: z.number().min(1) }),
  z.object({ kind: z.literal("conditionalScore"), tag: tagSchema, count: z.number().min(1), ifMet: z.number(), ifNot: z.number(), scope: z.enum(["owned", "hand"]).optional() }),
  ...
  z.object({ kind: z.literal("onPlayTagBudget"), tag: tagSchema, budget: z.number() }),
  z.object({ kind: z.literal("onPlayAnyTagCount"), points: z.number() }),
  z.object({ kind: z.literal("onTurnEndHandScore"), points: z.number() }),
  ...
  z.object({ kind: z.literal("settlementFloor"), pct: z.number().min(0).max(1) }),
  z.object({ kind: z.literal("playScoreFloor"), pct: z.number().min(0).max(1) }),
]);
